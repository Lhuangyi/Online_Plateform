...
class OrgDescView(View):
    def get(self, request, course_id):
        current_page = 'decs'
        course_org = CourseOrg.objects.filter(id = int(course_id))
        has_fav = False
        if request.user.is_authenticated():
...
class AddFavView(View):
    def post(self, request):
        ...
        if not request.user.is_authenticated():
            return HttpResponse({'status':'fail','msg':'User does not login'},content_type='application/json')
        ...
        if exist_records:
            ...
            if int(fav_id)>0 and int(fav_type)>0:
                ...
                if int(fav_type) == 1:
                    ...
                    teacher.fav_nums += 1
                    teacher.save()
                return HttpResponse('{"status":"success", "msg":"Already added"}', content_type='application/json')
            else:
    ...
    def get(self, request):
        ...
        if search_keywords:
            all_teachers = all_teachers.filter(Q(name__icontains=search_keywords)|
                                               ...
                                               Q(work_position_icontains=search_keywords))
        sort = request.GET.get('sort', '')
        if sort:
            if sort == "hot":
        ...
        return request(request,'teachers-list.html',{
